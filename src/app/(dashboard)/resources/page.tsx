...
function DbCard({
  ...
  const colorClass = CATEGORY_COLORS[db.category] ?? 'text-muted-foreground bg-muted'
  ...
  return (
    <div className="group flex flex-col rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      {/* Header */}
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <p className="text-sm font-semibold text-foreground leading-snug">{db.name}</p>
            <button
      ...
      <p className="px-4 pb-3 text-xs text-muted-foreground flex-1 leading-relaxed">
        {db.description}
      </p>
      ...
      <div className="flex gap-2 border-t border-border p-3">
        <button
          ...
          className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-muted transition-colors"
        >
...
export default function ResourcesPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-foreground">Bases de Dados Clínicas</h1>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-border bg-card pl-12 pr-12 py-3.5 text-sm text-foreground placeholder-gray-400 focus:border-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-800/20 shadow-sm"
        />
        {q && (
          <button
            onClick={() => setQ('')}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <select
          ...
          className="rounded-lg border border-border bg-card px-3 py-2 text-sm text-foreground focus:border-brand-800 focus:outline-none"
        >
      ...
      {recentDbs.length > 0 && !q && !filterCategory && !showOnlyFav && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-sm font-semibold text-foreground">Acessadas recentemente</p>
          </div>
            ...
            {recentDbs.map(db => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-sm text-foreground hover:bg-muted transition-colors shadow-sm"
                >
                  <Icon className="h-3.5 w-3.5 text-muted-foreground" />
                  {db.name}
                  <ExternalLink className="h-3 w-3 text-gray-300 dark:text-gray-600" />
                </button>
              )
            })}
          </div>
      ...
      {Object.keys(grouped).length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-16 text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground">Nenhuma base encontrada</p>
