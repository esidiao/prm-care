 ...
 * com window.open('', '_blank') e herda a origem da aplicação — nomes de
 * medicamentos e do farmacêutico são texto livre e não podem entrar crus.
 */
  ...
  const printConsult = () => {
    ...
    const rows = resp.interactions.map(i => {
      ...
      <div style="border:1px solid #e2e8f0;border-left:5px solid ${cor[i.severity]};border-radius:8px;padding:10px 14px;margin:8px 0">
        <b style="color:${cor[i.severity]}">[${esc(i.severityLabel)}]</b> <b>${esc(i.drugs[0])} + ${esc(i.drugs[1])}</b>${(i.evidenceLevel || ex?.evidenceLevel) ? ` <span style="font-size:11px;color:#64748b">(Evidência: ${esc(i.evidenceLevel || ex?.evidenceLevel)})</span>` : ''}
        <div style="font-size:13px;margin-top:4px"><b>Mecanismo:</b> ${esc(i.mechanism)}</div>
        <div style="font-size:13px"><b>Efeito clínico:</b> ${esc(i.clinicalEffect)}</div>
        <div style="font-size:13px"><b>Conduta:</b> ${esc(i.management)}</div>
        ${(i.monitoring || ex?.monitoring) ? `<div style="font-size:13px"><b>Monitorar:</b> ${esc(i.monitoring || ex?.monitoring)}</div>` : ''}
        ${(i.references && i.references.length) ? `<div style="font-size:12px;color:#475569"><b>Fontes:</b> ${i.references.map(esc).join(' · ')}</div>` : ''}
        ${(i.contextFlags || []).map(f => `<div style="font-size:12px;color:#92400e">⚠ Ajuste ao paciente: ${esc(f)}</div>`).join('')}
      </div>`
    }).join('')
    const fs = (resp.foodSupplements || []).map(f => `
      <div style="border:1px solid #e2e8f0;border-left:5px solid ${cor[f.severity]};border-radius:8px;padding:8px 14px;margin:6px 0">
        <b>${esc(f.emoji)} ${esc(f.agent)}</b> <span style="font-size:11px;color:#475569">[${esc(f.type)} · ${esc(f.severityLabel)}]</span>
        <div style="font-size:12px">Afeta: ${f.drugs.map(esc).join(', ')} — ${esc(f.clinicalEffect)}. <b>Conduta:</b> ${esc(f.management)}</div>
      </div>`).join('')
    ...
    w.document.write(`<!doctype html><meta charset="utf-8"><title>Consulta de Interações — PRM Care</title>
      ...
      <div style="border-bottom:3px solid #1e3a5f;padding-bottom:8px;margin-bottom:14px">
        ...
        <div style="color:#475569;font-size:13px">Método Dáder · ${esc(today)}</div>
      </div>
      <p style="margin:4px 0"><b>Medicamentos:</b> ${drugs.map(esc).join(', ')}${ctxLine ? ` &nbsp;·&nbsp; <b>Contexto:</b> ${esc(ctxLine)}` : ''}</p>
      <p style="margin:4px 0"><b>Farmacêutico(a):</b> ${esc(pharmacist)}</p>
      <p style="margin:8px 0"><b>Risco global:</b> ${esc(resp.globalLabel)}${resp.count ? ` · ${esc(resp.count)} interação(ões)` : ''}</p>
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-foreground">Interações Medicamentosas</h1>
        <a href="/interactions/history" className="text-sm font-medium text-teal-700 hover:underline">Histórico →</a>
      </div>
      <p className="mt-1 text-sm text-muted-foreground">Cruze dois ou mais medicamentos e veja as interações da base clínica do PRM Care.</p>
      ...
      <div className="mt-5 rounded-xl border border-border bg-card p-4">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-input px-3 py-2 text-sm outline-none focus:border-teal-500"
          />
          <button onClick={addDrug} className="flex items-center gap-1 rounded-lg bg-muted px-3 py-2 text-sm font-medium text-foreground hover:bg-accent">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 rounded-lg bg-muted px-3 py-2 text-sm">
          <span className="text-xs font-semibold text-muted-foreground">Contexto (opcional):</span>
          <label className="flex items-center gap-1 text-muted-foreground">Idade <input type="number" value={ctx.age} onChange={e => setCtx({ ...ctx, age: e.target.value })} className="w-16 rounded border border-input px-2 py-1" /></label>
          <label className="flex items-center gap-1 text-muted-foreground">TFG <input type="number" value={ctx.tfg} onChange={e => setCtx({ ...ctx, tfg: e.target.value })} className="w-16 rounded border border-input px-2 py-1" /> mL/min</label>
          <label className="flex items-center gap-1 text-muted-foreground"><input type="checkbox" checked={ctx.pregnant} onChange={e => setCtx({ ...ctx, pregnant: e.target.checked })} /> Gestante</label>
        </div>
        <div className="mt-4 flex items-center gap-3">
          ...
          {drugs.length < 2 && <span className="text-xs text-muted-foreground">Adicione ao menos 2 medicamentos.</span>}
        </div>
      </div>
        ...
        <div className="mt-6">
          ...
          {resp.notFound && (
            <div className="rounded-xl border border-border bg-muted px-4 py-4 text-sm text-muted-foreground">
              ...
              <span className="mt-1 block text-muted-foreground">Ausência de evidência não significa ausência de risco — mantenha o julgamento clínico.</span>
            </div>
          )}
          {resp.interactions.length > 0 && (
            <div className={`flex items-center justify-between rounded-xl border bg-card px-4 py-3 dark:border-gray-700 ${resp.globalRisk ? SEV[resp.globalRisk].ring : ''}`}>
              <div className="flex items-center gap-3">
                <AlertTriangle className={`h-5 w-5 ${resp.globalRisk === 'contraindicated' || resp.globalRisk === 'major' ? 'text-red-600' : 'text-amber-500'}`} />
                <div>
                  <div className="text-xs uppercase tracking-wide text-muted-foreground">Risco global da combinação</div>
                  <div className="text-lg font-bold text-foreground">{resp.globalLabel} · {resp.count} interação(ões)</div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={printConsult} className="flex items-center gap-1 rounded-lg border border-border px-3 py-1.5 text-sm text-muted-foreground hover:bg-muted">
                  <Printer className="h-4 w-4" /> Imprimir / PDF
                </button>
                <button onClick={copySummary} className="flex items-center gap-1 rounded-lg border border-border px-3 py-1.5 text-sm text-muted-foreground hover:bg-muted">
                ...
                <button onClick={saveConsult} disabled={saving || !!savedId} className="flex items-center gap-1 rounded-lg bg-brand-800 px-3 py-1.5 text-sm font-medium text-white disabled:opacity-50">
          ...
          <div className="mt-4 space-y-3">
            {resp.interactions.map((it, i) => {
              ...
              return (
                <div key={i} className={`relative overflow-hidden rounded-xl border bg-card p-4 dark:border-gray-700 ${s.ring}`}>
                  ...
                  <dl className="mt-2 space-y-1.5 text-sm">
                    ...
                    <div><dt className="inline font-semibold text-muted-foreground">Efeito clínico: </dt><dd className="inline text-foreground">{it.clinicalEffect}</dd></div>
                    <div><dt className="inline font-semibold text-muted-foreground">Conduta farmacêutica: </dt><dd className="inline text-foreground">{it.management}</dd></div>
                    {e?.warningSigns && <div><dt className="inline font-semibold text-muted-foreground">Sinais de alerta: </dt><dd className="inline text-foreground">{e.warningSigns}</dd></div>}
                    {(it.monitoring || e?.monitoring) && <div><dt className="inline font-semibold text-muted-foreground">Monitorar: </dt><dd className="inline text-foreground">{it.monitoring || e?.monitoring}</dd></div>}
                    {e?.alternatives && e.alternatives !== '—' && <div><dt className="inline font-semibold text-muted-foreground">Alternativas: </dt><dd className="inline text-foreground">{e.alternatives}</dd></div>}
                    {it.references && it.references.length > 0 && <div><dt className="inline font-semibold text-muted-foreground">Fontes: </dt><dd className="inline text-foreground">{it.references.join(' · ')}</dd></div>}
                  </dl>
          ...
          {resp.foodSupplements && resp.foodSupplements.length > 0 && (
            <div className="mt-5">
              <h3 className="mb-2 text-sm font-bold text-foreground">Alimentos, álcool e suplementos ({resp.foodSupplements.length})</h3>
              <div className="space-y-3">
                {resp.foodSupplements.map((f, i) => {
                  ...
                  return (
                    <div key={i} className={`relative overflow-hidden rounded-xl border bg-card p-4 dark:border-gray-700 ${c.ring}`}>
                      <div className={`absolute left-0 top-0 h-full w-1.5 ${c.bar}`} />
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-lg">{f.emoji}</span>
                        <span className="font-semibold text-foreground">{f.agent}</span>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">Afeta: {f.drugs.join(', ')}</p>
                      <dl className="mt-2 space-y-1.5 text-sm">
                        <div><dt className="inline font-semibold text-muted-foreground">Mecanismo: </dt><dd className="inline text-foreground">{f.mechanism}</dd></div>
                        <div><dt className="inline font-semibold text-muted-foreground">Efeito: </dt><dd className="inline text-foreground">{f.clinicalEffect}</dd></div>
                        <div><dt className="inline font-semibold text-muted-foreground">Conduta: </dt><dd className="inline text-foreground">{f.management}</dd></div>
                      </dl>
          ...
          {sources.length > 0 && (
            <div className="mt-4 rounded-lg border border-border bg-muted px-4 py-3 text-xs text-muted-foreground">
          ...
          {savedId && (
            <div className="mt-4 rounded-xl border border-border bg-card p-4">
              <h3 className="text-sm font-semibold text-foreground">Decisão clínica do farmacêutico</h3>
              <textarea
                value={dec.note} onChange={e => setDec({ ...dec, note: e.target.value })}
                placeholder="Conduta adotada / observações clínicas…"
                className="mt-2 w-full rounded-lg border border-input px-3 py-2 text-sm outline-none focus:border-teal-500" rows={3}
              />
              ...
              <input
                ...
                className="mt-2 w-full rounded-lg border border-input px-3 py-2 text-sm outline-none focus:border-teal-500"
              />
