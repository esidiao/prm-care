...
export default function NewAnalysisPage() {
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      await update() // refresh session token balance
      ...
      {step === 'patient' && (
        <div className="rounded-xl border bg-white p-6 shadow-sm space-y-4">
              ...
              {patients.map((p: any) => (
                <button key={p.id} onClick={() => setSelectedPatient(p)}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{p.code} · {p.age || (p.dateOfBirth ? new Date().getFullYear() - new Date(p.dateOfBirth).getFullYear() : null)} anos</p>
                  </div>
                  {selectedPatient?.id === p.id && (
                    <span className="rounded-full bg-[#1e3a5f] px-2.5 py-0.5 text-xs font-medium text-white">Selecionado</span>
                  )}
                </button>
              ))}
            </div>
          ...
          <div className="flex justify-between pt-2">
            ...
            <button onClick={() => selectedPatient && setStep('medications')} disabled={!selectedPatient}
        ...
        <div className="rounded-xl border bg-white p-6 shadow-sm space-y-4">
          ...
          {medications.length === 0 ? (
            <div className="rounded-lg border border-dashed py-10 text-center">
              <p className="text-gray-400">Nenhum medicamento adicionado.</p>
            ...
            <div className="space-y-4">
              {medications.map((med, i) => (
                <div key={med.id} className="rounded-lg border p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-semibold text-gray-700">Medicamento {i + 1}</span>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div>
                      ...
                      <input value={med.activeIngredient} onChange={e => updateMedication(med.id, 'activeIngredient', e.target.value)}
                        className="w-full rounded-md border px-3 py-2 text-sm focus:border-[#1e3a5f] focus:outline-none" placeholder="Ex: metformina" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Nome comercial</label>
                      <input value={med.tradeName} onChange={e => updateMedication(med.id, 'tradeName', e.target.value)}
                        className="w-full rounded-md border px-3 py-2 text-sm focus:border-[#1e3a5f] focus:outline-none" placeholder="Ex: Glifage" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Dose</label>
                      <div className="flex gap-1">
                        <input type="number" value={med.dose} onChange={e => updateMedication(med.id, 'dose', e.target.value)}
                          className="w-24 rounded-md border px-3 py-2 text-sm focus:border-[#1e3a5f] focus:outline-none" placeholder="500" />
                        <select value={med.doseUnit} onChange={e => updateMedication(med.id, 'doseUnit', e.target.value)}
                    ...
                    <div>
                      ...
                      <select value={med.route} onChange={e => updateMedication(med.id, 'route', e.target.value)}
                    ...
                    <div>
                      ...
                      <select value={med.frequency} onChange={e => updateMedication(med.id, 'frequency', e.target.value)}
                        className="w-full rounded-md border px-3 py-2 text-sm">
                    ...
                    <div>
                      ...
                      <select value={med.adherence} onChange={e => updateMedication(med.id, 'adherence', e.target.value)}
                        className="w-full rounded-md border px-3 py-2 text-sm">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input value={med.indication} onChange={e => updateMedication(med.id, 'indication', e.target.value)}
                        className="w-full rounded-md border px-3 py-2 text-sm focus:border-[#1e3a5f] focus:outline-none" placeholder="Ex: diabetes tipo 2, hipertensão..." />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-xs font-medium text-gray-600 mb-1">Efeitos adversos relatados</label>
                      <input value={med.adverseEffects} onChange={e => updateMedication(med.id, 'adverseEffects', e.target.value)}
                        className="w-full rounded-md border px-3 py-2 text-sm focus:border-[#1e3a5f] focus:outline-none" placeholder="Ex: náusea, tontura, custo elevado..." />
                    </div>
                    <div className="flex items-center gap-4">
                      <label className="flex items-center gap-2 text-sm cursor-pointer">
                        <input type="checkbox" checked={med.isSelfMedication} onChange={e => updateMedication(med.id, 'isSelfMedication', e.target.checked)}
                          className="h-4 w-4 rounded" />
                        Automedicação
                      </label>
                      <label className="flex items-center gap-2 text-sm cursor-pointer">
                        <input type="checkbox" checked={med.isPrescribed} onChange={e => updateMedication(med.id, 'isPrescribed', e.target.checked)}
                          className="h-4 w-4 rounded" />
                        Prescrito
                      </label>
                    </div>
          ...
          <div className="flex justify-between pt-2">
            ...
            <button onClick={() => setStep('clinical')}
        ...
        <div className="rounded-xl border bg-white p-6 shadow-sm space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <select onChange={e => setClinicalData((p: any) => ({ ...p, renalFunction: e.target.value }))}
                className="w-full rounded-md border px-3 py-2 text-sm">
            ...
            <div>
              ...
              <input type="number" onChange={e => setClinicalData((p: any) => ({ ...p, creatinineClearance: parseFloat(e.target.value) }))}
                className="w-full rounded-md border px-3 py-2 text-sm focus:border-[#1e3a5f] focus:outline-none" placeholder="Ex: 45" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Função hepática</label>
              <select onChange={e => setClinicalData((p: any) => ({ ...p, hepaticFunction: e.target.value }))}
                className="w-full rounded-md border px-3 py-2 text-sm">
                <option value="">Não informada</option>
                <option value="normal">Normal</option>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setClinicalData((p: any) => ({ ...p, labResults: [...(p.labResults || []), { examName: '', value: '', unit: '', isAbnormal: false }] }))}
                className="text-xs text-[#1e3a5f] hover:underline">+ Adicionar exame</button>
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border bg-white p-6 shadow-sm space-y-4">
            ...
            <div className="rounded-lg bg-gray-50 p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{medications.length} medicamento(s)</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Exames</span>
                <span className="font-medium">{clinicalData.labResults?.length || 0} exame(s)</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <button onClick={runAnalysis}
              disabled={isLoading || balance < (tokenCost?.cost ?? 0) || medications.length === 0}
