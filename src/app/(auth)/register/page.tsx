...
export default function RegisterPage() {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Left panel */}
      <div className="hidden lg:flex lg:w-5/12 flex-col bg-gradient-to-br from-brand-800 to-[#2563eb] p-12 text-white">
        ...
        <div className="w-full max-w-lg space-y-8">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-muted-foreground">Etapa {step}/2</span>
            </div>
            <div className="h-1.5 w-full rounded-full bg-muted">
              <div className="h-1.5 rounded-full bg-brand-800 transition-all" style={{ width: step === 1 ? '50%' : '100%' }} />
            </div>
          </div>
            ...
            {step === 1 && (
              <>
                <div>
                  ...
                  <input id="name" {...register('name')} className="w-full rounded-lg border border-border px-4 py-3 text-sm focus:border-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-800/20" placeholder="Dr. Maria Silva" />
                  {errors.name && <p className="mt-1 text-xs text-red-600">{errors.name.message}</p>}
                </div>

                <div>
                  <label htmlFor="email" className="block text-sm font-medium text-foreground mb-1">Email profissional</label>
                  <input id="email" {...register('email')} type="email" className="w-full rounded-lg border border-border px-4 py-3 text-sm focus:border-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-800/20" placeholder="seu@email.com" />
                  {errors.email && <p className="mt-1 text-xs text-red-600">{errors.email.message}</p>}
                </div>

                <div>
                  <label htmlFor="role" className="block text-sm font-medium text-foreground mb-1">Perfil de acesso</label>
                  <select id="role" {...register('role')} className="w-full rounded-lg border border-border px-4 py-3 text-sm focus:border-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-800/20">
                ...
                {role === 'PROFESSIONAL' && (
                  <div>
                    <label htmlFor="crfNumber" className="block text-sm font-medium text-foreground mb-1">Nº CRF (opcional)</label>
                    <input id="crfNumber" {...register('crfNumber')} className="w-full rounded-lg border border-border px-4 py-3 text-sm focus:border-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-800/20" placeholder="CRF/SP 00000" />
                  </div>
                )}

                <div>
                  <label htmlFor="institution" className="block text-sm font-medium text-foreground mb-1">Instituição (opcional)</label>
                  <input id="institution" {...register('institution')} className="w-full rounded-lg border border-border px-4 py-3 text-sm focus:border-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-800/20" placeholder="Hospital / Clínica / Universidade" />
                </div>

                <div>
                  <label htmlFor="password" className="block text-sm font-medium text-foreground mb-1">Senha</label>
                  <div className="relative">
                    <input id="password" {...register('password')} type={showPassword ? 'text' : 'password'} className="w-full rounded-lg border border-border px-4 py-3 pr-11 text-sm focus:border-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-800/20" placeholder="Mínimo 8 caracteres" />
                ...
                <div>
                  ...
                  <input id="confirmPassword" {...register('confirmPassword')} type="password" className="w-full rounded-lg border border-border px-4 py-3 text-sm focus:border-brand-800 focus:outline-none focus:ring-2 focus:ring-brand-800/20" placeholder="Repita a senha" />
                  {errors.confirmPassword && <p className="mt-1 text-xs text-red-600">{errors.confirmPassword.message}</p>}
                </div>

                <button type="button" onClick={goToStep2}
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-brand-800 py-3 text-sm font-semibold text-white hover:bg-brand-900 transition-colors">
              ...
              <>
                <div className="space-y-4">
                  ...
                  <div className="rounded-xl border border-border p-4">
                    <div className="mb-3 text-sm font-semibold text-foreground">Termos de Uso e Privacidade</div>
                    <div className="mb-4 h-32 overflow-y-auto rounded-lg bg-muted p-3 text-xs text-muted-foreground space-y-2">
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input type="checkbox" {...register('acceptTerms')} className="mt-0.5 h-4 w-4 rounded border-border text-brand-800" />
                      <span className="text-sm text-foreground">
                        Li e aceito os <Link href="/terms" target="_blank" className="text-brand-800 underline">Termos de Uso</Link>
                      </span>
                    </label>
                  ...
                  {/* Privacy */}
                  <div className="rounded-xl border border-border p-4">
                    <div className="mb-3 text-sm font-semibold text-foreground">Política de Privacidade e LGPD</div>
                    <div className="mb-4 h-24 overflow-y-auto rounded-lg bg-muted p-3 text-xs text-muted-foreground space-y-2">
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input type="checkbox" {...register('acceptPrivacy')} className="mt-0.5 h-4 w-4 rounded border-border text-brand-800" />
                      <span className="text-sm text-foreground">
                        Li e aceito a <Link href="/privacy" target="_blank" className="text-brand-800 underline">Política de Privacidade</Link> e o tratamento dos meus dados conforme a LGPD
                      </span>
                    </label>
                  ...
                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input type="checkbox" {...register('acceptClinicalDisclaimer')} className="mt-0.5 h-4 w-4 rounded border-border text-amber-600" />
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setStep(1)}
                    className="flex-1 rounded-lg border border-border py-3 text-sm font-semibold text-foreground hover:bg-muted transition-colors">
                    Voltar
                  </button>
                  <button type="submit" disabled={isLoading}
                    className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-brand-800 py-3 text-sm font-semibold text-white hover:bg-brand-900 disabled:opacity-60 transition-colors">
          ...
          <p className="text-center text-sm text-muted-foreground">
            Já tem conta?{' '}
            <Link href="/login" className="font-medium text-brand-800 hover:underline">Entrar</Link>
          </p>
        </div>
      </div>
    </div>
