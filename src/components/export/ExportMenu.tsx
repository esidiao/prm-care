    ...
    return (
      <div className="relative" ref={ref}>
        <button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-card text-muted-foreground hover:border-muted-foreground/40 hover:text-foreground transition-colors shadow-sm"
          disabled={anyLoading}
        >
          {anyLoading
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(o => !o)}
        disabled={anyLoading}
        className="flex items-center gap-2 rounded-lg border border-border bg-card px-3.5 py-2 text-sm font-medium text-muted-foreground shadow-sm hover:border-muted-foreground/40 hover:text-foreground transition-colors disabled:opacity-60"
      >
        {anyLoading
...
function DropdownPanel({
  ...
  return (
    <div className="absolute right-0 top-full z-50 mt-1.5 w-72 rounded-xl border border-border bg-card shadow-xl">
      <div className="border-b border-border px-4 py-2.5">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Exportar como CSV</p>
      </div>
        ...
        {options.map(opt => {
          ...
          return (
            <button
              ...
              disabled={s === 'loading'}
              className="flex w-full items-start gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-muted transition-colors disabled:opacity-60"
            >
              <div className="mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-md bg-muted">
                {s === 'loading' ? <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" /> : opt.icon}
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-foreground">{opt.label}</p>
                <p className="text-xs text-muted-foreground leading-tight mt-0.5">{opt.sublabel}</p>
              </div>
