...
export function PgxAlerts({ drugs }: { drugs: string[] }) {
  ...
  return (
    <div className="rounded-2xl border border-indigo-200 bg-card p-4 sm:p-6 print:hidden">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Medicamentos do paciente com diretriz gene–medicamento relevante. Considere genotipagem quando indicado.</p>
      <div className="mt-3 space-y-2">
        {items.map((g, i) => (
          <div key={i} className="rounded-lg border border-border bg-indigo-50/40 px-3 py-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-indigo-100 px-2 py-0.5 text-xs font-bold text-indigo-800">{g.gene}</span>
              <span className="text-sm font-semibold text-foreground">{g.drugLabel}</span>
              <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] text-muted-foreground">CPIC {g.level}</span>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">{g.summary}</p>
          </div>
        ))}
      </div>
      <p className="mt-3 text-xs text-muted-foreground">Diretrizes CPIC (cpicpgx.org) — apoio à decisão; a conduta depende do genótipo/fenótipo real, exames e contexto clínico. Não substitui aconselhamento genético.</p>
    </div>
  )
}
