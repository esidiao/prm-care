              ...
              {results!.prms.length > 0 && (
                <section className="p-2">
                  ...
                  {results!.prms.map(f => (
                    <button
                      key={f.id}
                      onClick={() => navigate(`/analysis/${f.analysisId}`)}
                      className="flex w-full items-start gap-2.5 rounded-lg px-2 py-2.5 text-left hover:bg-blue-50 transition-colors"
                    >
                      <span className="mt-0.5 flex-shrink-0">{RISK_ICONS[f.riskLevel]}</span>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-gray-800">{f.title}</p>
              ...
              {results!.medications.length > 0 && (
                <section className="p-2">
                  ...
                  {results!.medications.map(m => (
                    <button
                      key={m.id}
                      onClick={() => navigate(`/patients/${m.patientId}`)}
                      className="flex w-full items-center gap-2.5 rounded-lg px-2 py-2.5 text-left hover:bg-blue-50 transition-colors"
                    >
                      <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-md bg-emerald-50">
